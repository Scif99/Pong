...
void Ball::Reset(int start_dir)
{
    ...
    m_velocity_.x = start_dir ? -5.f : 5.f; //Right player scores : Left player scores
    m_velocity_.y = 0.f;
}
...
void HandleCollisions(const Paddle& paddle, Ball& ball)
{
    if (ball.m_shape_.getGlobalBounds().intersects(paddle.m_shape_.getGlobalBounds()))
    {
        ...
        //Update x velocity
        ball.m_velocity_.x = (1.1f) * std::abs(ball.m_velocity_.x); //Increase magnitude
        ball.m_velocity_.x *= paddle.m_side_ == Paddle::Side::LEFT ? 1.f : -1.f; //Now decide direction

        ball.m_velocity_.y = ball.m_shape_.getPosition().y < paddle.m_shape_.getPosition().y ? -5.f : 5.f;
